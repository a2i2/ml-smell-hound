...
def get_command_output(command, is_text=True, cmd_input=None, print=False, my_env={**os.environ}):
    if print:
        cargs = command_print(command.split())
    else:
        cargs = command.split()
    return subprocess.run(args=cargs, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=is_text, input=cmd_input, env=my_env)
